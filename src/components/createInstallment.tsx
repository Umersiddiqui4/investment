...
const installmentFormSchema = z.object({
  ...
  preferredInstallmentDate: z.string().optional(),
...
export function CreateInstallmentForm() {
  ...
  const form = useForm<InstallmentFormValues>({
    ...
    defaultValues: {
      ...
      preferredInstallmentDate: new Date().toISOString().split("T")[0],
      costPrice: "",
      sellPrice: "",
  ...
  const handleProfileImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: "investor" | "customer") => {
    const file = e.target.files?.[0]
    if (!file) return
  ...
  const createInstallments = (sellPrice: any, totalPayments: any, startDate: any) => {
    const amount = Number.parseFloat(sellPrice) / Number.parseInt(totalPayments)
    const installments = []
    ...
    <div className="container mx-auto py-6 overflow-x-hidden">
      <div className="flex flex-col space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Create Sale</h1>
        </div>
                                        ...
                                        {localInvestors.map((investor: any) => (
                                          ...
                                          >
                                            ...
                                            <CheckCircle2
                                              className={cn(
                                                "ml-auto h-4 w-4",
                                                field.value.includes(investor.id)
                            ...
                            <Popover>
                              ...
                              <PopoverContent className="w-full p-0 border-2 border-cyan-200 dark:border-cyan-800 shadow-lg shadow-cyan-100 dark:shadow-cyan-900/20 min-w-[300px]">
                                <Command>
                                  ...
                                  <CommandGroup>
                                    <CommandList className="max-h-[350px] overflow-y-auto py-2 cursor-pointer overscroll-contain">
                                      {localCustomers.map((customer: any) => (
                                        ...
                                        >
                                          ...
                                          <CheckCircle2
                                            className={cn(
                                              "ml-auto h-4 w-4",
                                              customer.id === field.value ? "opacity-100 text-cyan-500" : "opacity-0",
                                            )}
                                          />
                                        </CommandItem>
                                      ))}
                                  ...
                                  <div className="mt-1 space-y-2">
                                    ...
                                    >
                                      {guarantorCnicBackPreviews[index] ? (
                                        <img
                                          src={
                                            guarantorCnicBackPreviews[index] || "/placeholder.svg" || "/placeholder.svg"
                                          }
                                          alt="CNIC Back"
                                          className="w-full h-full object-contain p-2"
                                        />
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <FormField
                          control={form.control}
                          name="preferredInstallmentDate"
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Preferred Installment Date</FormLabel>
                              <FormControl>
                                <Input
                                  type="date"
                                  {...field}
                              ...
                              {localInvestors
                                .filter(
                                  (investor: any) =>
                                    ...
                                    !investors.some((selected: any) => selected.id === investor.id),
                                )
                                .map((investor: any) => (
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center bg-slate-100 dark:bg-slate-800 p-3 rounded-lg">
                              ...
                              <span className="font-bold text-lg text-emerald-600 dark:text-emerald-400">
                                ...
                                {investors
                                  .reduce(
                                    (sum: any, investor: any) => sum + (Number.parseFloat(investor.contribution) || 0),
                                    0,
                                  )
                                  .toFixed(2)}
                              </span>
                    ...
                    {form.getValues("customerId") ? (
                      <div className="flex items-center gap-2 p-2 bg-cyan-50 dark:bg-cyan-900/20 rounded-md">
                        <Avatar className="h-6 w-6">
                          <img
                            src={
                              localCustomers.find((c: any) => c.id === form.getValues("customerId"))?.image ||
                              "/placeholder.svg" ||
                              "/placeholder.svg" ||
                              "/placeholder.svg" ||
                              "/placeholder.svg"
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm">
                            {localCustomers.find((c: any) => c.id === form.getValues("customerId"))?.name}
                          </span>
                          <span className="text-xs text-slate-500 dark:text-slate-400">
                            {localCustomers.find((c: any) => c.id === form.getValues("customerId"))?.email}
                          </span>
                        </div>
                      </div>
                    ) : (
      ...
      <AnimatePresence>
        {showAddInvestor && (
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex justify-end overflow-hidden">
            <motion.div
